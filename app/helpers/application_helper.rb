module ApplicationHelper
  ...
  def apps_navigation
    if Rails.cache.read("apps_navigation").to_s == ''
      data = HTTParty.get Setting.uapp_url + '/api/v1/site_navigation.json'
      Rails.cache.write("apps_navigation", data.parsed_response, :expires_in => 10.seconds)
    end
    Rails.cache.read("apps_navigation")
  end

  def apps_menu
    if Rails.cache.read("apps_menu").to_s == ''
      data = HTTParty.get Setting.uapp_url + '/api/v1/site_menu.json'
      Rails.cache.write("apps_menu", data.parsed_response, :expires_in => 10.seconds)
    end
